...
using static BetterJoyForDolphin.HIDapi;
...
namespace BetterJoyForDolphin {
	public class JoyconManager {
        ...
        static string[] dolphinPorts = { (ConfigurationManager.AppSettings["DolphinPort1"]).ToString(), (ConfigurationManager.AppSettings["DolphinPort2"]).ToString(), (ConfigurationManager.AppSettings["DolphinPort2"]).ToString(), (ConfigurationManager.AppSettings["DolphinPort4"]).ToString() };
		...
		void CleanUp() { // removes dropped controllers from list
			...
			for (int i = 0; i < j.Count; i++) {
				...
				if (v.state == Joycon.state_.DROPPED) {
                    ...
                    // remove Config event handler
                    foreach (Button b in form.configBtn)
                    {

                        if (b.Enabled & b.Tag == v)
                        {
                            b.Click -= ((Joycon)b.Tag).configAction;
		...
		public void CheckForNewControllers() {
            ...
            while (ptr != IntPtr.Zero) {
                ...
                DolphinJoyConfig dolphinJoyConfig = Program.generic_manager.Contains(enumerate.serial_number);

                if (dolphinJoyConfig.serial_number != "-1") {
                    enumerate.product_id = dolphinJoyConfig.product_id;
                }
					...
					if (j.Count < 5) {
						...
						foreach (Button v in form.con) {
							...
							if (!v.Enabled) {
                                ...
                                form.configBtn[ii].Invoke(new MethodInvoker(delegate {
                                form.configBtn[ii].Tag = j.Last();
                                form.configBtn[ii].Enabled = true;

                                j.Last().configAction = delegate (object sender, EventArgs e)
                                { form.configBtnClick(sender, e, (Joycon)form.configBtn[ii].Tag); };

                                form.configBtn[ii].Click += ((Joycon)form.configBtn[ii].Tag).configAction;
                                }));

                                int port = -1;
                                
                                bool success = int.TryParse(dolphinPorts[ii], out port);

                                j.Last().SetPack(port, dolphinJoyConfig.force, dolphinJoyConfig.inverse);
	...
	class Program {
        ...
        public static GenericControllerManager generic_manager;
		...
		public static void Start() {
            ...
            if (useHIDG) {
				...
				try {
					response = (HttpWebResponse)WebRequest.Create(@"http://localhost:26762/api/v1/hidguardian/whitelist/add/" + pid).GetResponse(); // add BetterJoyForDolphin to allowed processes 
				} catch (Exception e) {
		...
		public static void Stop() {
			...
			if (Boolean.Parse(ConfigurationManager.AppSettings["PurgeAffectedDevices"])) {
				try {
					HttpWebResponse r1 = (HttpWebResponse)WebRequest.Create(@"http://localhost:26762/api/v1/hidguardian/affected/purge/").GetResponse();
				} catch { Console.WriteLine("program Error 1"); }
			}
